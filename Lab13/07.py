            ...
            for ca in range(len(ls[ra])):   # for column
                if (ra == r and ca == c) and (ls[r][c] != "*"):
                    ...
                    if r-1 >= 0:      # upper
                        if ls[r-1][c] != "*":
                            ls[r-1][c] += 1
                    if (r-1 >= 0 and c-1 >= 0):     # top left corner
                        if ls[r-1][c-1] != "*":
                            ls[r-1][c-1] += 1
                    if r-1 >= 0 and c+1 < mn[1]: # top right corner
                        if ls[r-1][c+1] != "*":
                            ls[r-1][c+1] += 1
                    if r+1 < mn[0]:                # lower
                        if ls[r+1][c] != "*":
                            ls[r+1][c] += 1
                    if r+1 < mn[0] and c-1 >= 0: # lower left corner
                        if ls[r+1][c-1] != "*":
                            ls[r+1][c-1] += 1
                    if (r+1 < mn[0] and c+1 < mn[1]): # lower right corner
                        if ls[r+1][c+1] != "*":
                            ls[r+1][c+1] += 1
